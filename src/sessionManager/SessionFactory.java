...
public class SessionFactory implements ISession 
{
		...
		try {
		    ...
		    for(Method method : cls.getMethods())
		    {
		    	if(method.isAnnotationPresent(OneToOne.class))
		    	{
		    		...
		    		if(methodHasValue(obj, field)) continue;
		    		
		    		SessionFactory session = new SessionFactory(this.config);
			...
			{	
				...
				{				
					...
					if (method.getReturnType() == boolean.class){ preparedStatement.setBoolean(parameterIndex, (boolean)method.invoke(obj)); parameterIndex ++; continue;}
					if (method.getReturnType() == int.class){ preparedStatement.setInt(parameterIndex, (int)method.invoke(obj)); parameterIndex ++; continue;}
					if (method.getReturnType() == double.class){preparedStatement.setDouble(parameterIndex, (double)method.invoke(obj)); parameterIndex ++; continue;}
					if (method.getReturnType() == String.class){ preparedStatement.setString(parameterIndex, (String)method.invoke(obj)); parameterIndex ++; continue;}
					if (method.getReturnType() == short.class){preparedStatement.setShort(parameterIndex, (short)method.invoke(obj)); parameterIndex ++; continue;}
					if (method.getReturnType() == long.class){ preparedStatement.setLong(parameterIndex, (long)method.invoke(obj)); parameterIndex ++; continue;}
					if (method.getReturnType() == float.class){ preparedStatement.setFloat(parameterIndex, (float)method.invoke(obj)); parameterIndex ++; continue;}
					...
					if (method.getReturnType() == Date.class)
					{
						...
						preparedStatement.setDate(parameterIndex, dt, calendar); parameterIndex ++; continue;	
					} 
				}
			}
			...
			{	
				if(method.getName().contains("get") && !method.getName().contains("class Test")&& !method.getName().contains("Class"))
				{
					...
					if(method.isAnnotationPresent(Version.class))
					{
						...
						preparedStatement.setInt(parameterIndex, (version + 1)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == boolean.class)
					{
						System.out.println(method.getName());
						preparedStatement.setBoolean(parameterIndex, (boolean)method.invoke(obj)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == int.class)
					{
						System.out.println(method.getName());
						preparedStatement.setInt(parameterIndex, (int)method.invoke(obj)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == double.class)
					{
						System.out.println(method.getName());
						preparedStatement.setDouble(parameterIndex, (double)method.invoke(obj)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == String.class)
					{
						System.out.println(method.getName());
						preparedStatement.setString(parameterIndex, (String)method.invoke(obj)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == short.class)
					{
						System.out.println(method.getName());
						preparedStatement.setShort(parameterIndex, (short)method.invoke(obj)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == long.class)
					{
						System.out.println(method.getName());
						preparedStatement.setLong(parameterIndex, (long)method.invoke(obj)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == float.class)
					{
						System.out.println(method.getName());
						preparedStatement.setFloat(parameterIndex, (float)method.invoke(obj)); parameterIndex ++; continue;
					}
					
					if (method.getReturnType() == byte.class)
					{
						System.out.println(method.getName());
						preparedStatement.setByte(parameterIndex, (byte)method.invoke(obj)); parameterIndex ++; continue;
					}
					//if (method.getReturnType() == byte.class){ preparedStatement.setByte(parameterIndex, (byte)method.invoke(obj)); parameterIndex ++; continue;}
	...
	public void onID(Object obj, int id) 
	{
		...
		try {
			...
			for(Method method : cls.getMethods())
			{
				if(method.isAnnotationPresent(OneToOne.class))
				{
					...
					if(oneToOne.load() == LOAD.MANUAL) continue;
			...
			while(resultSet.next())
			{
				...
				{							
					if(method.getName().contains("set") && !method.getName().contains("class Test")&& !method.getName().contains("Class"))
					{
						...
						if(oneToOneMtd.isAnnotationPresent(OneToOne.class)) continue;
		...
		{
			if(statement != null) clostSTMT(statement);
		}
	}
